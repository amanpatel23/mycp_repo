...
private class CF732 {
    fun solveTestCase(): Unit {
        ...
        val decrement = mutableListOf<Int>()
        val increment = mutableListOf<Int>()

        for (i in 0 until n) {
            val diff = a[i] - b[i]
            if (diff > 0) {
                for (j in 0 until abs(diff)) {
                    decrement.add(i + 1)
                }
            } else if (diff < 0) {
                for (j in 0 until abs(diff)) {
        ...
        val decLen = decrement.size
        val incLen = increment.size
        if (decLen == incLen) {
            outputWriter.println(decLen)
            for (i in 0 until decLen) {
                outputWriter.println("${decrement[i]} ${increment[i]}")
            }
        } else
