...
int t_count, m_count, t_exp, m_exp;

bool _check(string s, int n) {
    t_count = 0, m_count = 0;
    rep(i, 0, n) {
        if (s[i] == 'T')
            t_count++;
...
void solve() {
    ...
    t_exp = ((n * 2) / 3), m_exp = (n / 3);
    string s_inv = "";
    rep(i, 0, n) {
        s_inv.insert(0, 1, s[i]);
    }

    if (_check(s, n) && _check(s_inv, n)) {
        if (t_count == t_exp && m_count == m_exp)
            yes;
        else
            no;
    } else
