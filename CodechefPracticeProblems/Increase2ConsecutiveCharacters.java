...
public class Increase2ConsecutiveCharacters {
    ...
    private static void solve() {
        ...
        for (int i = 1; i <= n; i++) {
            int diff = (b.charAt(i - 1) - a.charAt(i - 1) + 26) % 26;
            if ((i & 1) == 1) prefix_sum[i] = prefix_sum[i - 1] + diff;
            else prefix_sum[i] = prefix_sum[i - 1] - diff;
        }

        while (q-- > 0) {
            int l = fs.nextInt(), r = fs.nextInt();
            if ((prefix_sum[r] - prefix_sum[l - 1]) % 26 == 0) fw.out.println("YES");
