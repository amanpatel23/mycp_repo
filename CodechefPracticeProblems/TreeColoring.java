...
public class TreeColoring {
    ...
    private static void solve() {
        ...
        for (int i = 0; i < n - 1; i++) {
            addEdge(fs.nextInt() - 1, fs.nextInt() - 1, adjList);
        }
        ...
        while (!qq.isEmpty()) {
            int[] front = qq.poll();
            vis[front[0]] = true;
            long temp = (front[1] == 0 ? (c - 1) : (c - 2));
            for (int x : adjList.get(front[0])) {
                if (vis[x]) continue;
                qq.add(new int[]{x, front[1] + 1});
                ans = (ans * temp) % mod1;
                temp--;
            }
        }
    ...
    private static void addEdge(int a, int b, List<List<Integer>> adjList) {
        adjList.get(a).add(b);
        adjList.get(b).add(a);
    }
