...
public class CF767 {
    ...
    private static void solve() {
        ...
        Integer[] require = readIntArray(n);
        Integer[] will_give = readIntArray(n);

        List<Integer> list = IntStream.range(0, n).boxed().sorted(Comparator.comparingInt(i -> require[i])).collect(Collectors.toList());
        long result = k;
        for (int i = 0; i < n; i++) {
            int idx = list.get(i);
            if (require[idx] > k)
                break;
            result += will_give[idx];
            k += will_give[idx];
        }
