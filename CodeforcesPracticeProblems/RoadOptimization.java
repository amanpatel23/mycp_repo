...
public class RoadOptimization {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt(), l = fs.nextInt(), k = fs.nextInt();
        int[] coord = new int[n + 1];
        int[] speed_limit = new int[n];
        for (int i = 0; i < n; i++)
            coord[i] = fs.nextInt();
        for (int i = 0; i < n; i++)
            speed_limit[i] = fs.nextInt();
        coord[n] = l;

        long[][] dp = new long[n + 1][k + 1];
        for (int i = 0; i <= n; i++)
            Arrays.fill(dp[i], lMax);
        dp[0][0] = 0;
        ...
        for (int j = 0; j <= k; j++) {
            for (int i = 0; i < n; i++) {
                ...
                for (int x = 0; x <= (k - j); x++) {
                    ...
                    if (i_new > n)
                        break;
                    int dist = coord[i_new] - coord[i];
                    long time = (long) dist * speed_limit[i];
                    dp[i_new][j + x] = Math.min(dp[i_new][j + x], dp[i][j] + time);
                }
            }
        }
