...
public class PalindromeBasis {
    ...
    private static final int maxV = (int) (4e4);
    private static int list_len;
    ...
    private static void solve() {
        ...
        fw.out.println(dp[list_len][fs.nextInt()]);
    }
    ...
    private static void preCalc() {
        ...
        for (int i = 1; i <= maxV; i++) {
            ...
            if (isPalindrome(s, 0, s.length() - 1)) list.add(i);
        }
        list_len = list.size();

        dp = new long[list_len + 5][maxV + 5];
        Arrays.fill(dp[0], 0);
        for (int i = 0; i <= list_len; i++) dp[i][0] = 1;
        for (int i = 1; i <= list_len; i++) {
            for (int j = 1; j <= maxV; j++) {
                dp[i][j] = dp[i - 1][j];
                if (j >= list.get(i - 1)) dp[i][j] = (dp[i][j] + dp[i][j - list.get(i - 1)]) % mod1;
            }
        }
    }
