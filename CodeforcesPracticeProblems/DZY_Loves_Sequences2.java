...
public class DZY_Loves_Sequences2 {
    ...
    private static void solve() {
        ...
        int[] left_to_right = new int[n], right_to_left = new int[n];
        int prev = 0, len = 0;
        for (int i = 0; i < n; i++) {
            left_to_right[i] = 1;
            if (arr[i] > prev)
        ...
        for (int i = 0; i < n; i++) {
            if (i > 0 && i < n - 1 && arr[i + 1] - arr[i - 1] >= 2)
                ans = Math.max(ans, 1 + left_to_right[i - 1] + right_to_left[i + 1]);
            if (i > 0) ans = Math.max(ans, 1 + left_to_right[i - 1]);
            if (i < n - 1) ans = Math.max(ans, 1 + right_to_left[i + 1]);
        }
        fw.out.println(ans);
    }
