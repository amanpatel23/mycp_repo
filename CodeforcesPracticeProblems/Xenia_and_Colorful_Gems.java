...
public class Xenia_and_Colorful_Gems {
    ...
    private static final long lMax = Long.MAX_VALUE, lMin = (long) (-1e18 - 100);
    ...
    private static void solve() {
        ...
        int nr = fs.nextInt(), ng = fs.nextInt(), nb = fs.nextInt();
        List<Integer> red = readIntList(nr);
        List<Integer> green = readIntList(ng);
        List<Integer> blue = readIntList(nb);

        Collections.sort(red);
        Collections.sort(green);
        Collections.sort(blue);

        long ans = getMin(getAns(red, green, blue),
                getAns(green, red, blue),
                getAns(blue, red, green));
        fw.out.println(ans);
    }

    private static long getAns(List<Integer> list1, List<Integer> list2, List<Integer> list3) {
        ...
        for (int x : list1) {
            ...
            ans = getMin(ans, util(x, arr[0], arr[2]), util(x, arr[0], arr[3]),
                    util(x, arr[1], arr[2]), util(x, arr[1], arr[3]));
        }
        return ans;
    }

    private static int bs1(int x, List<Integer> list) {
        ...
        return (list.get(r) <= x ? list.get(r) : list.get(l));
    }

    private static int bs2(int x, List<Integer> list) {
        int l = 0, r = list.size() - 1;
        while (r - l > 1) {
            int mid = l + ((r - l) / 2);
        ...
        return (list.get(l) >= x ? list.get(l) : list.get(r));
    }

    private static long util(long x, long y, long z) {
        long a = (x - y) * (x - y);
        long b = (y - z) * (y - z);
        long c = (z - x) * (z - x);
