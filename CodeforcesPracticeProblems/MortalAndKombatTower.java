...
public class MortalKombatTower2 {
    ...
    private static void solve() {
        ...
        int[][] dp = new int[n][2];
        // 0 -> killed by my friend and 1 -> killed by me
        dp[0][0] = arr[0];
        dp[0][1] = iMax;
        for (int i = 1; i < n; i++) {
            dp[i][0] = Math.min(dp[i - 1][1] + arr[i], (i - 2 >= 0 ? dp[i - 2][1] : 0) + arr[i - 1] + arr[i]);
            dp[i][1] = Math.min(dp[i - 1][0], (i - 2 >= 0 ? dp[i - 2][0] : iMax));
        }

        fw.out.println(Math.min(dp[n - 1][0], dp[n - 1][1]));
    }
