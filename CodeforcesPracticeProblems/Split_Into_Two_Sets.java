...
public class aaVirtual_CF_805 {
    ...
    private static void solve() {
       ...
       for (int i = 1; i <= n; i++) {
           if (color_assigned[i] == -1 && !isBipartite(i, 0, adjList, color_assigned)) {
               fw.out.println("NO");
               return;
           }
       }
    ...
    private static boolean isBipartite(int curr_node, int curr_color, List<List<Integer>> adjList, int[] color_assigned) {
        if (color_assigned[curr_node] != -1) {
            return (color_assigned[curr_node] == curr_color);
        }
        color_assigned[curr_node] = curr_color;
        for (int node : adjList.get(curr_node)) {
            if (!isBipartite(node, curr_color ^ 1, adjList, color_assigned)) {
                return false;
            }
        }
        return true;
