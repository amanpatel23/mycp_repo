...
public class Min_Max_Array_Transformation {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt();
        int[] arrff = readIntArray(n);
        int[] arrss = readIntArray(n);
        int[] ansff = new int[n];
        int[] ansss = new int[n];
        for (int i = 0; i < n; i++) {
            int l = 0, r = i;
            while (r - l > 1) {
                int mid = l + ((r - l) / 2);
                int diff = arrss[mid] - arrff[i];
                if (diff >= 0) r = mid;
                else l = mid;
            }
            int idx = (arrss[l] - arrff[i] >= 0 ? l : r);
            ansff[i] = arrss[idx] - arrff[i];
        }

        int[] preifx = new int[n];
        for (int i = 1; i < n; i++) {
            preifx[i] = (arrss[i - 1] - arrff[i] < 0 ? 1 : 0) + preifx[i - 1];
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            while (r - l > 1) {
                int mid = l + ((r - l) / 2);
                int cnt = preifx[mid] - preifx[i];
                if (cnt == 0) l = mid;
                else r = mid;
            }
            int idx = preifx[r] - preifx[i] == 0 ? r : l;
            ansss[i] = arrss[idx] - arrff[i];
        }

        for (int xx : ansff) fw.out.print(xx + " ");
        fw.out.println();
        for (int xx : ansss) fw.out.print(xx + " ");
        fw.out.println();
    }
