...
public class RestoreModulo {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt();
        Integer[] nums = readIntArray(n);

        int max = nums[0];
        Set<Integer> nonNegativeSet = new HashSet<>();
        Set<Integer> negativeSet = new HashSet<>();
        for (int i = 1; i < n; i++) {
            max = Math.max(max, nums[i]);
            int diff = nums[i] - nums[i - 1];
            if (diff >= 0)
                nonNegativeSet.add(diff);
            else
                negativeSet.add(diff);
        }

        if (nonNegativeSet.size() > 1 || negativeSet.size() > 1) {
            fw.out.println(-1);
            return;
        }

        if (nonNegativeSet.size() == 0 || negativeSet.size() == 0) {
            fw.out.println(0);
            return;
        }

        int m = nonNegativeSet.iterator().next() + Math.abs(negativeSet.iterator().next());
        if (m <= max) {
            fw.out.println(-1);
            return;
        }

        fw.out.println(m + " " + nonNegativeSet.iterator().next());
    }
