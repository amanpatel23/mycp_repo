...
public class Close_Tuples {
    ...
    private static Calc_nCr ncr;
    public static void main(String[] args) throws IOException {

        fs = new FastScanner();
        fw = new FastWriter();

        ncr = new Calc_nCr((int) (2e5 + 5), mod1);
        int t = 1;
        t = fs.nextInt();
        while (t-- > 0) {
            solve();
    ...
    private static void solve() {
        ...
        if (m == 2) {
            ...
            long ans = 0;
            while (i < n) {
                while (j + 1 < n && list.get(j + 1) - list.get(i) <= k) j++;
                ans = (ans + (j - i)) % mod1;
                i++;
            }
            fw.out.println(ans);
        ...
        long[] prefix_sum = new long[n + 5];
        for (int i = 1; i <= n; i++) {
            prefix_sum[i] = (ncr.nCr(i, m - 2) + prefix_sum[i - 1]) % mod1;
        }
        long ans = 0;
        int i = 0, j = 0;
        while (i < n) {
            while (j + 1 < n && list.get(j + 1) - list.get(i) <= k) j++;
            int in_between = j - i - 1;
            if (in_between >= 0)
                ans = (ans + prefix_sum[in_between]) % mod1;
            i++;
        }
        fw.out.println(ans);
    }
