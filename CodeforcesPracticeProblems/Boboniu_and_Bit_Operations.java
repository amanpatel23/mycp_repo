...
public class Boboniu_and_Bit_Operations {
    ...
    private static void solve() {
        ...
        int[] a_arr = readIntArray(n), b_arr = readIntArray(m);
        int[][] arr_2d = new int[n][m];
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                arr_2d[i][j] = a_arr[i] & b_arr[j];
            }
        }

        for (int ans = 0; ans <= (1 << 9); ans++) {
            boolean can_be_ans = true;
            for (int[] arr : arr_2d) {
                boolean flag = false;
                for (int x : arr) {
                    if ((ans | x) == ans) {
