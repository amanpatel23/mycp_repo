...
public class Prefix_Suffix_Palindrome {
    ...
    private static void solve() {
        ...
        String rev_substr = new StringBuilder(substr).reverse().toString();
        int max1 = iMax, max2 = iMax;
        for (int k = 0; k < 10; k++) {
            int num = (int) (Math.random() * 100);
            max1 = Math.min(max1, util(substr, num));
        }
        for (int k = 0; k < 10; k++) {
            int num = (int) (Math.random() * 100);
            max2 = Math.min(max2, util(rev_substr, num));
        }
        StringBuilder ans = new StringBuilder();
        for (int ii = 0; ii < i; ii++) ans.append(s.charAt(ii));
        if (max1 >= max2) {
            ans.append(substr, 0, max1);
        } else {
            String temp = rev_substr.substring(0, max2);
            ans.append(new StringBuilder(temp).reverse());
        }
        for (int ii = j + 1; ii < n; ii++) ans.append(s.charAt(ii));
        fw.out.println(ans);
    }

    private static int util(String str, int p) {
        int n = str.length();
        int max_palindrome = 0;
        long power = 1;
        long hash1 = 0, hash2 = 0;
        for (int ii = 0; ii < n; ii++) {
            int ascii = str.charAt(ii) - 'a' + 1;
            hash1 = (hash1 + ((ascii * power) % mod1)) % mod1;
            power = (power * p) % mod1;

            hash2 = (hash2 * p) % mod1;
            hash2 = (hash2 + ascii) % mod1;

            if (hash1 == hash2) max_palindrome = ii + 1;
        }
