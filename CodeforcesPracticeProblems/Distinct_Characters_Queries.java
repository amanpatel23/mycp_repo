...
public class Distinct_Characters_Queries {
    ...
    private static void solve() {
        ...
        while (q-- > 0) {
            ...
            if (t == 1) {
                int qi = fs.nextInt();
                char qc = fs.next().charAt(0);
                st.update(1, 1, n, qi, qc);
            }else {
                int l = fs.nextInt(), r = fs.nextInt();
                int ans = noOfSetBits(st.query(1, 1, n, l, r));
                fw.out.println(ans);
            }
        }
    }
    ...
    private static class SegmentTree {
        ...
        private void update(int si, int ss, int se, int qi, char qc) {
            if (ss == se) {
                arr[ss - 1] = qc;
                st[si] = (1 << (qc - 'a'));
                return;
            ...
            int mid = ss + ((se - ss) / 2);
            if (qi <= mid)
                update(2 * si, ss, mid, qi, qc);
            else
                update((2 * si) + 1, mid + 1, se, qi, qc);
