...
public class Checkposts {
    ...
    private static void solve() {
        ...
        SCC scc = new SCC(n, adjList);
        List<List<Integer>> scc_list = scc.getSCC();
        long ans = 0, ways = 1;
        for (List<Integer> x : scc_list) {
            if (x.size() == 0) continue;
            x.sort(Comparator.comparingInt(i -> costs[i]));
            int min = costs[x.get(0)];
            int cnt = 0;
            for (int idx : x) {
                if (costs[idx] == min) cnt++;
                else break;
            }
            ans += min;
            ways = (ways * cnt) % mod1;
        }
        fw.out.println(ans + " " + ways);
    }
