...
public class CF_815 {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                if (grid[i][j] == '1') {
                    ...
                    i + 1 < n && grid[i + 1][j] == '0') flag = true;
                    if (j - 1 >= 0 && grid[i][j - 1] == '0' &&
                    i - 1 >= 0 && grid[i - 1][j] == '0') flag = true;
                    if (j + 1 < m && grid[i][j + 1] == '0' &&
                    i - 1 >= 0 && grid[i - 1][j] == '0') flag = true;
                    if (i + 1 < n && grid[i + 1][j] == '0' &&
                            j - 1 >= 0 && grid[i][j - 1] == '0') flag = true;
                }
                if (grid[i][j] == '0') {
                    zeroes++;
                    if (j + 1 < m && grid[i][j + 1] == '0') flag = true;
                    if (i + 1 < n && grid[i + 1][j] == '0') flag = true;
                }
            }
        }
        int ones = (n * m) - zeroes;
    ...
    private static int util(char[][] grid, int i, int j) {
        int cnt = 0;
        if (grid[i][j] == '1') cnt++;
        if (grid[i][j + 1] == '1') cnt++;
        if (grid[i + 1][j] == '1') cnt++;
    ...
    private static void util2(char[][] grid, int i, int j) {
        grid[i][j] = '0';
        grid[i][j + 1] = '0';
        grid[i + 1][j] = '0';
        grid[i + 1][j + 1] = '0';
