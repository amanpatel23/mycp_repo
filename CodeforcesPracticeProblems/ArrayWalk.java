...
public class ArrayWalk {
    ...
    private static void solve() {
        ...
        dp = new int[k + 1][2][z + 1];
        for (int[][] row : dp)
            for (int[] r : row) Arrays.fill(r, -1);
        int result = maxScore(0, k, 0, z);
        fw.out.println(result);
    }

    private static int maxScore(int pos, int k, int left_succ, int z) {

        if (pos < 0 || pos >= n || left_succ > 1 || z < 0)
            return iMin;
        if (k == 0)
            return arr[pos];

        if (dp[k][left_succ][z] != -1)
            return dp[k][left_succ][z];

        int ans = arr[pos];
        ans += Math.max(maxScore(pos - 1, k - 1, left_succ + 1, z - 1),
                        maxScore(pos + 1, k - 1, 0, z));

        return dp[k][left_succ][z] = ans;
    }
