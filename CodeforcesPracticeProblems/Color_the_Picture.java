...
public class Color_the_Picture {
    ...
    private static void solve() {
        ...
        for (int num : arr) {
            int div = num / n;
            if (div > 1) list1.add(div);
            div = num / m;
            if (div > 1) list2.add(div);
        }

        if (util(list1, m) || util(list2, n)) {
            fw.out.println("YES");
        } else {
            fw.out.println("NO");
        }
    ...
    private static boolean util(List<Integer> list, long x) {
        long sum = 0;
        ...
        for (int num : list) {
            long temp = Math.min(num - 2, diff);
            diff -= temp;
            if (diff >= 2 && (num - temp) == 2) diff -= 2;
