...
public class Palindromic_Paths {
    ...
    private static void solve() {
        ...
        for (int i = 0; i < n; i++) matrix[i] = readIntArray(m);
        List<int[]> list = new ArrayList<>();
        int c = 0;
        while (true) {
            int r = Math.max(0, c - m + 1);
            if (r >= n) break;
            int[] arr = new int[]{0, 0};
            for (int i = r, j = Math.min(c, m - 1); i >= 0 && i < n && j >= 0 && j < m; i++, j--) {
                arr[matrix[i][j]]++;
            }
        ...
        while (j > i) {
            int z = list.get(i)[0] + list.get(j)[0];
            int o = list.get(i)[1] + list.get(j)[1];
            ans += Math.min(z, o);
