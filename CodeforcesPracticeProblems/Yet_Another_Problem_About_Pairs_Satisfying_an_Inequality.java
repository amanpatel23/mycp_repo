...
public class AA1_CF_Round_806 {
    ...
    private static void solve() {
        ...
        for (int i = n; i >= 1; i--) {
            ...
            prefix_sum[i] = prefix_sum[i + 1] + cnt;
        }

        long[] prefix_sum2 = new long[n + 5];
        for (int i = n; i >= 1; i--) {
            prefix_sum2[i] = prefix_sum2[i + 1];
            if (arr[i - 1] >= i) prefix_sum2[i]++;
        }

        long ans = 0;
        for (int i = 1; i <= n; i++) {
            if (i <= arr[i - 1]) continue;
            ans += (Math.max(0, prefix_sum[i + 1] - prefix_sum2[i + 1]));
        }
        fw.out.println(ans);
    }
