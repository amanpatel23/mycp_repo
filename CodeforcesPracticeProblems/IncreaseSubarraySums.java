...
public class CF_Edu123 {
    ...
    private static void solve() {
        ...
        Long[] subarray_sum = new Long[n + 1];
        Arrays.fill(subarray_sum, lMin);
        long max = 0;
        for (int i = 0; i < n; i++) {
            long sum = 0;
            for (int j = i; j < n; j++) {
                ...
                int len = j - i + 1;
                subarray_sum[len] = Math.max(subarray_sum[len], sum);
            }
        }
        //displayArr(subarray_sum);
        ...
        for (int i = 1; i <= n; i++) {
            max = result[i - 1];
            for (int j = i; j <= n; j++) {
                max = Math.max(max, subarray_sum[j] + ((long) i * x));
            }
            result[i] = max;
        }
