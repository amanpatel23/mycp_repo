...
public class MortalKombatTower {
    ...
    private static void solve() {
        ...
        int result = minSkips(0, 0);
        fw.out.println(result);
    }

    private static int minSkips(int i, int turn) {
        ...
        if (turn == 1)
            return dp[turn][i] = Math.min(minSkips(i + 1, 0),
                    minSkips(i + 2, 0));

        return dp[turn][i] = Math.min(arr[i] + minSkips(i + 1, 1),
                arr[i] + (i < (n - 1) ? arr[i + 1] : 0) + minSkips(i + 2, 1));
    }
