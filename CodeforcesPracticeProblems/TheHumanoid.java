...
public class TheHumanoid {
    ...
    private static void solve() {
        ...
        int ans = (int) getMax(util(i, arr, pow, new int[]{2, 3, 2}),
                util(i, arr, pow, new int[]{3, 2, 2}),
                util(i, arr, pow, new int[]{2, 2, 3}));
        fw.out.println(ans);
    }

    private static int util(int i, int[] arr, long pow, int[] seq) {
        int idx = 0;
        while (i < arr.length) {
            if (pow <= arr[i]) {
                if (idx == 3) break;
