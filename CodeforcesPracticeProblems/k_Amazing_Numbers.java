...
public class k_Amazing_Numbers {
    ...
    private static void solve() {
        ...
        for (int k : map.keySet()) map.get(k).add(n);

        int[] ans = new int[n + 1];
        Arrays.fill(ans, iMax);
        for (int k : map.keySet()) {
            int dist = -1;
            for (int i = 0; i < map.get(k).size() - 1; i++) {
                dist = Math.max(dist, map.get(k).get(i + 1) - map.get(k).get(i));
            }
        ...
        for (int i = 1; i <= n; i++) {
            ...
            if (ans[i] == iMax) fw.out.print(-1 + " ");
            else fw.out.print(ans[i] + " ");
        }
        fw.out.println();
    }
