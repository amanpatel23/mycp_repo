...
public class Weight_of_the_System_of_Nested_Segments {
    ...
    private static void solve() {
        ...
        String faltu = fs.nextLine();
        int n = fs.nextInt(), m = fs.nextInt();
        List<int[]> list = new ArrayList<>();
        for (int i = 1; i <= m; i++)
            list.add(new int[]{i, fs.nextInt(), fs.nextInt()});
        ...
        fw.out.println(sum);
        list2.sort(Comparator.comparingInt(xx -> xx[1]));
        int i = 0, j = (2 * n) - 1;
        while (j > i) {
            fw.out.println(list2.get(i)[0] + " " + list2.get(j)[0]);
            j--; i++;
        }
        fw.out.println();
    }
