...
public class A_and_B {
    ...
    private static void solve() {
        ...
        int[] arr = new int[2];
        arr[0] = fs.nextInt(); arr[1] = fs.nextInt();
        if (arr[0] > arr[1]) swapInIntArr(arr, 0, 1);
        long l = 0, r = (long) (1e5);
        while (r - l > 1) {
            long mid = l + ((r - l) / 2);
            if (arr[0] + ((mid * (mid + 1)) / 2) >= arr[1])
                r = mid;
            else
                l = mid;
        }
        long n;
        if (arr[0] + ((l * (l + 1)) / 2) >= arr[1]) n = l;
        else n = r;

        long a_new = arr[0] + ((n * (n + 1)) / 2);
        long diff = a_new - arr[1];
        if ((diff & 1) == 0) {
            fw.out.println(n);
        }else {
            if (((a_new & 1) == 0 && (n & 1) == 0) || ((a_new & 1) == 1 && (n & 1) == 0)) {
                fw.out.println(n + 1);
            }else {
                fw.out.println(n + 2);
            }
