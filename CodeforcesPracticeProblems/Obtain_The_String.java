...
public class Obtain_The_String {
    ...
    private static void solve() {
        ...
        List<List<Integer>> s_list = new ArrayList<>(), t_list = new ArrayList<>();
        for (int i = 0; i < 26; i++) {
            s_list.add(new ArrayList<>());
            t_list.add(new ArrayList<>());
        }
        for (int i = 0; i < s.length(); i++) {
            int idx = s.charAt(i) - 'a';
            s_list.get(idx).add(i);
        }
        for (int i = 0; i < t.length(); i++) {
            int idx = t.charAt(i) - 'a';
            t_list.get(idx).add(i);
        }

        for (int i = 0; i < 26; i++) {
            if (t_list.get(i).size() > 0 && s_list.get(i).size() == 0) {
                fw.out.println(-1);
                return;
            }
        }
        ...
        for (char c : t.toCharArray()) {
            int idx = c - 'a';
            int next = bs(s_list.get(idx), x);
            if (next < x) {
                ans++;
                x = s_list.get(idx).get(0) + 1;
            } else
                x = next + 1;
        }
