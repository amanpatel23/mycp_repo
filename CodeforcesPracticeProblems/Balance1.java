...
public class CF_830_Div2 {
    ...
    private static void solve() {
        ...
        Map<Long, Long> mm1 = new HashMap<>();
        Map<Long, List<Long>> mm2 = new HashMap<>();
        while (q-- > 0) {
            String[] arr = fs.nextLine().split(" ");
            char sign = arr[0].charAt(0);
            long num = Long.parseLong(arr[1]);
            if (sign == '+') {
                set.add(num);
                if (mm2.containsKey(num)) {
                    for (long xx : mm2.get(num)) {
                        long vv = num;
                        while (set.contains(vv)) vv += xx;
                        mm1.put(xx, vv);
                        if (!mm2.containsKey(vv)) mm2.put(vv, new ArrayList<>());
                        mm2.get(vv).add(xx);
                    }
                    mm2.remove(num);
                }
            } else {
                if (mm1.containsKey(num)) {
                    fw.out.println(mm1.get(num));
                } else {
                    for (long ii = 1; ; ii++) {
                        long vv = num * ii;
                        if (!set.contains(vv)) {
