...
public class Minimum_Grid_Path {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt();
        int[] costs = readIntArray(n);
        long[] ans1 = util(costs, n, 0);
        long[] ans2 = util(costs, n, 1);
        long ans = lMax;
        for (int i = 0; i < n; i++) {
            if (ans1[i] == -1) break;
            if (i > 0 && ans2[i - 1] != -1) ans = Math.min(ans, ans1[i] + ans2[i - 1]);
            if (ans2[i] != -1) ans = Math.min(ans, ans1[i] + ans2[i]);
        }
        fw.out.println(ans);
    }

    private static long[] util(int[] costs, int n, int st) {
        long[] ans = new long[n];
        Arrays.fill(ans, -1);
        long sum = 0;
        long min = lMax;
