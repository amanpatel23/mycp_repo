...
public class Journey {
    ...
    private static void solve() {
        ...
        double ans = 0;
        Queue<double[]> qq = new LinkedList<>();
        qq.offer(new double[]{1, 0, 1});
        boolean[] visited = new boolean[n + 1];
        while (!qq.isEmpty()) {

            double[] front = qq.poll();
            visited[(int) front[0]] = true;
            int no_of_children = adjList.get((int) front[0]).size();
            if ((int) front[0] != 1) no_of_children--;
            if (no_of_children == 0) {
                ans += (front[1] * front[2]);
                continue;
            }
            double nextlen = front[1] + 1, nextprob = front[2] / no_of_children;
            for (int x : adjList.get((int) front[0])) {
                if (!visited[x]) {
                    qq.offer(new double[]{x, nextlen, nextprob});
                }
            }
        }
