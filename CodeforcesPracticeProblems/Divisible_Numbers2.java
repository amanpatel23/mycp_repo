...
public class Divisible_Number2 {
    ...
    private static void solve() {
        ...
        List<int[]> aafactors = new ArrayList<>();
        List<int[]> bbfactors = new ArrayList<>();
        util(a, aafactors);
        util(b, bbfactors);

        for (int[] aa : aafactors) {
            for (int[] bb: bbfactors) {
                long ff = (long) aa[0] * bb[0];
                long ss = (long) aa[1] * bb[1];

                long div = a / ff;
                long x = -1;
                for (long ii = div; ii < div + 5; ii++) {
                    long temp = ff * ii;
                    if (temp > a && temp <= c) {
                        x = temp;
                        break;
    ...
    private static void util(int num, List<int[]> list) {
