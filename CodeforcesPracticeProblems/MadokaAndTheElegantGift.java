...
public class MadokaAndTheElegantGift {
    ...
    private static List<List<Integer>> rows, cols;
    private static Set<Integer> r_set, c_set;
    ...
    private static void solve() {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < m; j++) {
                ...
                int check = rows.get(r_set.iterator().next()).size();
                for (int x : r_set) {
                    if (rows.get(x).size() != check) {
                        fw.out.println("NO");
                        return;
                    }
                }
                check = cols.get(c_set.iterator().next()).size();
                for (int x : c_set) {
                    if (cols.get(x).size() != check) {
                        fw.out.println("NO");
                        return;
                    }
                }
