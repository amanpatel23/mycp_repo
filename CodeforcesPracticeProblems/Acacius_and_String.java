...
public class Acacius_and_String {
    ...
    private static final String to_match = "abacaba";
    ...
    private static void solve() {
        ...
        for (int i = 0; i + 6 < n; i++) {
            ...
            if (set.contains(substr)) {
                ...
                if (i > 0) s.append(str, 0, i);
                s.append(to_match);
                if (i + 7 < n) s.append(str, i + 7, n);

                int ans = util(s.toString(), n);
                if (ans == 1) {
                    fw.out.println("YES");
                    for (int j = 0; j < n; j++) {
                        if (s.charAt(j) == '?') s.setCharAt(j, 'e');
                    }
                    fw.out.println(s);
                    return;
                }
    ...
    private static int util(String s, int n) {
        int cnt = 0;
        for (int i = 0; i + 6 < n; i++) {
            String substr = s.substring(i, i + 7);
            if (substr.equals(to_match)) cnt++;
        }
        return cnt;
    }

    private static void precalc(int i, String s) {
        if (i == 7) {
            set.add(s);
            return;
        }
        precalc(i + 1, s + to_match.charAt(i));
        precalc(i + 1, s + '?');
    }
