...
public class Sequence_and_Swaps {
    ...
    private static void solve() {
        ...
        int[][] dp = new int[n + 5][505];
        for (int[] r : dp) Arrays.fill(r, -1);
        int ans = min_swaps(1, x, n, arr, dp);
        fw.out.println((ans == iMax) ? -1 : ans);
    }

    private static int min_swaps(int i, int x, int n, int[] arr, int[][] dp) {
        if (i > n) return 0;

        if (dp[i][x] != -1) return dp[i][x];
        int ans;
        if (arr[i] < arr[i - 1]) ans = iMax;
        else {
            ans = min_swaps(i + 1, x, n, arr, dp);
            if (arr[i] > x && arr[i - 1] <= x) {
                int temp = arr[i];
                arr[i] = x;
                ans = Math.min(ans, 1 + min_swaps(i + 1, temp, n, arr, dp));
            }
        }
        return dp[i][x] = ans;
    }
