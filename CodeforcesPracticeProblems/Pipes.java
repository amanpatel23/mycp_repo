...
public class Pipes {
    ...
    private static void solve() {
        ...
        grid[0] = fs.nextLine().toCharArray();
        grid[1] = fs.nextLine().toCharArray();

        ans = false;
        dfs(0, 0, n, grid, 0);
        fw.out.println(ans ? "YES" : "NO");
    }

    private static void dfs(int r, int c, int n, char[][] grid, int dir) {
        if (r == 1 && c == n) {
            ans = true;
            return;
        }
        if (r < 0 || r >= 2 || c < 0 || c >= n) return;
