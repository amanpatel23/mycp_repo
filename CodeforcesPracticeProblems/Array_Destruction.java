...
public class Array_Destruction {
    ...
    private static void solve() {
        ...
        for (int i = 1; i < (2 * n); i++) {
            ...
            ans.add(new int[]{list.get(0), list.get(i)});
            for (int ii = 1; ii < (2 * n); ii++) {
                if (ii == i) continue;
                int num = list.get(ii);
                int diff = x - num;

                if (!update(map, num)) continue;
                if (!update(map, diff)) continue;

                ans.add(new int[]{num, diff});
                x = Math.max(num, diff);
            }

            if (ans.size() == n) {
                fw.out.println("YES");
                fw.out.println(list.get(0) + list.get(i));
                for (int[] arr : ans) fw.out.println(arr[0] + " " + arr[1]);
                return;
            }
        }
        fw.out.println("NO");
    ...
    private static void put(Map<Integer, Integer> map, int k) {
        map.put(k, map.getOrDefault(k, 0) + 1);
    }

    private static boolean update(Map<Integer, Integer> map, int k) {
