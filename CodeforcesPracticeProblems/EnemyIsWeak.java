...
public class EnemyIsWeak {
    ...
    private static List<Integer> nums, nums_copy;
    private static long[] fw_tree1, fw_tree2;
    ...
    private static void solve() {
        ...
        fw_tree1 = new long[n + 1];
        fw_tree2 = new long[n + 1];
        Arrays.fill(fw_tree1, 0L);
        Arrays.fill(fw_tree2, 0L);

        long result = 0;
        for (int i = (n - 1); i >= 0; i--) {
            int x = arr[i];
            result += get(x - 1, fw_tree2);
            long count = get(x - 1, fw_tree1);
            update(x, fw_tree1, 1);
            update(x, fw_tree2, count);
        }

        fw.out.println(result);
    }

    private static long get(int x, long[] fw_tree) {
    ...
    private static void update(int x, long[] fw_tree, long add) {
