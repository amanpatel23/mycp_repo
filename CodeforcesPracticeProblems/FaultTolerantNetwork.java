...
public class FaultTolerantNetwork {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt();
        int[] row1 = new int[n], row2 = new int[n];
        for (int i = 0; i < n; i++)
            row1[i] = fs.nextInt();
        for (int i = 0; i < n; i++)
            row2[i] = fs.nextInt();

        long first_to_first = Math.abs(row1[0] - row2[0]);
        long first_to_last = Math.abs(row1[0] - row2[n - 1]);
        long last_to_first = Math.abs(row1[n - 1] - row2[0]);
        long last_to_last = Math.abs(row1[n - 1] - row2[n - 1]);

        if (n == 1) {
            fw.out.println(first_to_first);
            return;
        }

        long row1_first = lMax, row1_last = lMax, row2_first = lMax, row2_last = lMax;
        for (int i = 0; i < n; i++) {
            row1_first = Math.min(row1_first, Math.abs(row1[0] - row2[i]));
            row1_last = Math.min(row1_last, Math.abs(row1[n - 1] - row2[i]));

            row2_first = Math.min(row2_first, Math.abs(row2[0] - row1[i]));
            row2_last = Math.min(row2_last, Math.abs(row2[n - 1] - row1[i]));
        }

        long result = getMin(first_to_first + last_to_last,
                first_to_last + last_to_first,
                first_to_first + row1_last + row2_last,
                first_to_last + row1_last + row2_first,
                last_to_first + row1_first + row2_last,
                last_to_last + row1_first + row2_first,
                row1_first + row1_last + row2_first + row2_last);
