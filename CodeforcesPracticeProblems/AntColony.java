...
public class AntColony {
    ...
    private static int[] candidate, freq;
    ...
    private static void solve() {
        ...
        candidate = new int[(4 * n) + 5];
        Arrays.fill(candidate, iMax);
        freq = new int[(4 * n) + 5];
        ...
        while (q-- > 0) {
            ...
            int[] result_arr = query(1, 1, n, l, r);
            int result = (r - l + 1) - result_arr[1];
    ...
    private static void buildTree(int si, int ss, int se) {
        ...
        int gcd = gcd(candidate[2 * si], candidate[(2 * si) + 1]);
        candidate[si] = gcd;
        if (candidate[2 * si] == gcd)
            freq[si] += freq[2 * si];
        if (candidate[(2 * si) + 1] == gcd)
            freq[si] += freq[(2 * si) + 1];
    }
    ...
    private static int[] query(int si, int ss, int se, int qs, int qe) {
        ...
        if (ss >= qs && se <= qe)
            return (new int[]{candidate[si], freq[si]});
    ...
    private static boolean util(int b, int a) {
        return (b % a == 0);
    }
