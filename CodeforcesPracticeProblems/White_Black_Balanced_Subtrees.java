...
public class Whit_Black_Balance_Subtrees {
    ...
    private static void solve() {
        ...
        for (int i = 2; i <= n; i++) {
            parents[i] = fs.nextInt();
        }
        String color = fs.nextLine();
    ...
    private static long[] dfs(int curr, List<List<Integer>> adjList, String color) {

        long[] arr = new long[]{0, 0};
        if (color.charAt(curr - 1) == 'W') arr[0]++;
        else arr[1]++;
        for (int x : adjList.get(curr)) {
            long[] temp = dfs(x, adjList, color);
            arr[0] += temp[0];
            arr[1] += temp[1];
        }
