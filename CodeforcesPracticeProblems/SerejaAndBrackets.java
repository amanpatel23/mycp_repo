...
public class SerejaAndBrackets {
    ...
    private static int[] st1, st2, st3;
    ...
    private static void solve() {
        ...
        while (q-- > 0) {
            ...
            Tuple result = query(1, 1, n, l, r);
            fw.out.println(result.x);
        }
    }
    ...
    private static void buildTree(int si, int ss, int se) {
        ...
        int min = (int) getMin(st2[2 * si], st3[(2 * si) + 1]);
        st1[si] = st1[2 * si] + st1[(2 * si) + 1] + (2 * min);
        st2[si] = st2[2 * si] - min + st2[(2 * si) + 1];
        st3[si] = st3[2 * si] + st3[(2 * si) + 1] - min;
    }

    private static Tuple query(int si, int ss, int se, int qs, int qe) {
        if (qs > se || qe < ss)
            return (new Tuple(0, 0, 0));

        if (ss >= qs && se <= qe)
            return (new Tuple(st1[si], st2[si], st3[si]));

        int mid = ss + ((se - ss) / 2);
        Tuple l = query(2 * si, ss, mid, qs, qe);
        Tuple r = query((2 * si) + 1, mid + 1, se, qs, qe);

        int min = (int) getMin(l.y, r.z);
        int x = l.x + r.x + (2 * min);
        int y = l.y - min + r.y;
        int z = l.z + r.z - min;
