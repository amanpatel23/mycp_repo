...
public class Circular_Local_MiniMax {
    ...
    private static void solve() {
        ...
        for (int i = 0; i < n; i += 2) {
            int max = Math.max(result[i + 1], result[(i - 1 + n) % n]);
            if (map.higherKey(max) == null) {
                fw.out.println("NO");
                return;
            }
            int k = map.higherKey(max);
            result[i] = k;
            util(k, map);
        }

        fw.out.println("YES");
        for (int x : result) fw.out.print(x + " ");
        fw.out.println();
    }

    private static void util(int k, TreeMap<Integer, Integer> map) {
