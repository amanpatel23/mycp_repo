...
public class Dolce_Vita {
    ...
    private static void solve() {
        ...
        int[] prices = readIntArray(n);
        randomizeIntArr(prices, n);
        Arrays.sort(prices);

        long sum = 0, ans = 0;
        for (int i = 0; i < n; i++) {
            ans += bs(i, sum, x, prices);
            sum += prices[i];
        }
        fw.out.println(ans);
    }

    private static int bs(int idx, long sum, long x, int[] prices) {
        int l = 1, r = (int) (1e9);
        while (r - l > 1) {
            int mid = l + ((r - l) / 2);
            if (check(mid, sum, x, idx, prices)) l = mid;
            else r = mid;
        }
        if (check(r, sum, x, idx, prices)) return r;
        else if (check(l, sum, x, idx, prices)) return l;
        return 0;
    }

    private static boolean check(long mid, long sum, long x, int idx, int[] prices) {
        long curr = sum + ((mid - 1) * idx);
        long p = prices[idx] + mid - 1;
        return (x - curr >= p);
    }
