...
public class Game_with_Chips {
    ...
    private static void solve() {
        ...
        result.append("L".repeat(m - 1)).append("U".repeat(n - 1));
        int turn = 0;
        for (int i = 0; i < n; i++) {
            if (turn == 0) result.append("R".repeat(m - 1));
            else result.append("L".repeat(m - 1));
            turn ^= 1;
            result.append("D");
        }


        fw.out.println(result.length());
        fw.out.println(result);
    }

    private static int util(int x, int max) {
        x = Math.max(0, x);
        x = Math.min(x, max);
        return x;
    }
