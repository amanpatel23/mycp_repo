...
public class NP_Hard_Problem {
    ...
    private static void solve() {
        ...
        int[] color_assigned = new int[n + 1];
        Arrays.fill(color_assigned, -1);
        for (int i = 1; i <= n; i++) {
            if (color_assigned[i] == -1 && !dfs(i, 0, adjList, color_assigned)) {
                fw.out.println(-1);
                return;
            }
        }

        List<Integer> set_A = new ArrayList<>(), set_B = new ArrayList<>();
        for (int i = 1; i <= n; i++) {
            if (color_assigned[i] == 0) set_A.add(i);
            else set_B.add(i);
        }

        fw.out.println(set_A.size());
        for (int x : set_A) fw.out.print(x + " ");
        fw.out.println();
        fw.out.println(set_B.size());
        for (int x : set_B) fw.out.print(x + " ");
    }

    private static boolean dfs(int curr_node, int curr_color, List<List<Integer>> adjList, int[] color_assigned) {

        color_assigned[curr_node] = curr_color;
        for (int x : adjList.get(curr_node)) {
            if (color_assigned[x] == -1 && !dfs(x, curr_color ^ 1, adjList, color_assigned)) return false;
            else if (color_assigned[x] == curr_color) return false;
        }
