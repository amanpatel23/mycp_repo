...
public class Vertical_Paths {
    ...
    private static void solve() {
        ...
        for (int node : bfs) {
            ...
            dfs(node, adjList, vis, path);
            ans.add(path);
        }

        fw.out.println(ans.size());
        for (List<Integer> temp : ans) {
            fw.out.println(temp.size());
            for (int xx : temp) {
                fw.out.print(xx + " ");
            }
            fw.out.println();
    ...
    private static void dfs(int node, List<List<Integer>> adjList, boolean[] vis, List<Integer> path) {
        vis[node] = true;
        path.add(node);
        if (adjList.get(node).size() != 0)
            dfs(adjList.get(node).get(0), adjList, vis, path);
    }
