...
public class They_Are_Everywhere {
    ...
    private static void solve() {
        ...
        while (r - l > 1) {
            int mid = l + ((r - l) / 2);
            if (check(mid, s, n, size)) r = mid;
            else l = mid;
        }
        int ans = check(l, s, n, size) ? l : r;
        fw.out.println(ans);
    }

    private static boolean check(int x, String s, int n, int size) {
    ...
    private static void add(Map<Character, Integer> map, char c) {
        map.put(c, map.getOrDefault(c, 0) + 1);
    }

    private static void remove(Map<Character, Integer> map, char c) {
