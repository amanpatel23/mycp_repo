...
public class Zmei_Gorynich {
    ...
    private static void solve() {
        ...
        list.sort(Comparator.comparingInt(i -> arr[i][1] - arr[i][0]));
        int idx1 = list.get(0);
        list.sort(Comparator.comparingInt(i -> -arr[i][0]));
        int idx2 = list.get(0);
        ...
        if (arr[idx1][1] - arr[idx1][0] >= 0) {
            fw.out.println(-1);
            return;
        }
        long ff = x - arr[idx1][0] + arr[idx1][1];
        long diff = ff - x;
        long temp = ceilDiv(Math.abs(arr[idx2][0] - ff + diff), Math.abs(diff));
        long tn = ff + ((temp - 1) * diff);
        if (tn <= 0) fw.out.println(temp);
        else fw.out.println(temp + 1);
    }
