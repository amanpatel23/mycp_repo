...
public class SlavicGsFavoriteProblem {
    ...
    private static void dfs1(int curr, int par, int xor, int b) {
        set.add(xor);
        for (int[] xx : adjList.get(curr)) {
            if (xx[0] == b || xx[0] == par) continue;
            dfs1(xx[0], curr, xor ^ xx[1], b);
        }
    }

    private static boolean dfs2(int curr, int par, int xor) {
        if (set.contains(xor)) return true;
        for (int[] xx : adjList.get(curr)) {
            if (xx[0] == par) continue;
            if (dfs2(xx[0], curr, xor ^ xx[1])) return true;
        }
        return false;
    }
