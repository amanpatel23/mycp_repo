...
public class MakeThemEqual {
    ...
    private static Integer[] b, c;
    private static long[][] dp = new long[1005][1005];
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i <= 1000; i++)
            Arrays.fill(dp[i], -1);
        operations = new long[10005];
        for (int i = 1; i <= 1000; i++) {
            operations[i] = minOp(1, i);
        }
    ...
    private static void solve() {
        ...
        long[] dp2 = new long[k + 5];
        long[] temp = new long[k + 5];
        for (int i = 1; i <= n; i++) {
            for (int j = 0; j <= k; j++) {
                temp[j] = dp2[j];
                if (cost[i - 1] > j) {
                    ...
                    dp2[j] = Math.max(c[i - 1] + temp[j - (int) (cost[i - 1])], temp[j]);
                }
            }
        }
    ...
    private static long minOp(int curr, int target) {
        ...
        for (int x = 1; x <= curr; x++) {
            ans = Math.min(ans, 1 + minOp(curr + (curr / x), target));
        }

        return dp[curr][target] = ans;
    }
