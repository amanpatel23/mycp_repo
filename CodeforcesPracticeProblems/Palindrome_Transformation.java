...
public class Palindromic_Transformation {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt(), p = fs.nextInt() - 1;
        char[] arr = fs.nextLine().toCharArray();
        long ans = 0;
        int i = 0, j = n - 1;
        while (i < j) {
            int ascii_1 = arr[i] - 'a', ascii_2 = arr[j] - 'a';
            ans += Math.min((ascii_1 - ascii_2 + 26) % 26, (ascii_2 - ascii_1 + 26) % 26);
            i++; j--;
        }
        int div = n / 2;
        int idx1 = 0, idx2 = div - 1;
        while (idx1 < div && arr[idx1] == arr[n - 1 - idx1]) idx1++;
        while (idx2 >= 0 && arr[idx2] == arr[n - 1 - idx2]) idx2--;
        if (idx1 > idx2) {
            fw.out.println(0);
            return;
        }
        if (p >= div) p = n - 1 - p;
        if (p <= idx1) ans += (idx2 - p);
        else if (p >= idx2) ans += (p - idx1);
        else {
            int min = Math.min(p - idx1, idx2 - p);
            int max = Math.max(p - idx1, idx2 - p);
            ans += (min + min + max);
        }
        fw.out.println(ans);
    }
