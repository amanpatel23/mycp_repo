...
public class CF_Round_835_Div4 {
    ...
    private static void solve() {
        ...
        int d = fs.nextInt();
        int[] arr = new int[n + 1];
        List<Integer> list = readIntList(n);
        list.sort(Collections.reverseOrder());
        for (int i = 1; i <= n; i++) arr[i] = list.get(i - 1);
        long temp = 0;
        for (int i = 1; i <= Math.min(n, d); i++) temp += arr[i];
        if (temp >= c) {
            fw.out.println("Infinity");
            return;
        }
        int l = 0, r = Math.max(0, d - 2);
        while (r - l > 1) {
            int mid = l + ((r - l) / 2);
            if (check(arr, n, mid, c, d)) l = mid;
            else r = mid;
        }
        if (check(arr, n, r, c, d)) {
            fw.out.println(r);
            return;
        }
        if (check(arr, n, l, c, d)) {
            fw.out.println(l);
            return;
        }
    ...
    private static boolean check(int[] arr, int n, int k, long c, int d) {
        long sum = 0;
    ...
    private static boolean check(int[] arr, int l, int r) {
        int ll = arr[l], rr = arr[r];
        boolean ff = (l == 0) || (arr[l - 1] > ll);
        boolean ss = (r == arr.length - 1) || (arr[r + 1] > rr);
        return ff && ss;
    }
