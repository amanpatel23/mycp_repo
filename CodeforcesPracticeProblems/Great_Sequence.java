...
public class Great_Sequence {
    ...
    private static void put(TreeMap<Long, Integer> tm, long num) {
        tm.put(num, tm.getOrDefault(num, 0) + 1);
    }

    private static int update(TreeMap<Long, Integer> tm, long ff, long ss) {
        if (ff == ss) {
            if (tm.get(ff) >= 2) {
                tm.put(ff, tm.get(ff) - 2);
                if (tm.get(ff) == 0) tm.remove(ff);
                return 0;
            } else {
                tm.remove(ff);
                return 1;
        ...
        if (tm.getOrDefault(ss, 0) >= 1) {
            tm.put(ff, tm.get(ff) - 1);
            tm.put(ss, tm.getOrDefault(ss, 0) - 1);
            if (tm.get(ff) == 0) tm.remove(ff);
            if (tm.get(ss) <= 0) tm.remove(ss);
            return 0;
        }
