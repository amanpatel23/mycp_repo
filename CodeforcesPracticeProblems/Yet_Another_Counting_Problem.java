...
public class Yet_Another_Counting_Problem {
    ...
    private static void solve() {
        ...
        b = Math.max(a, b); a = min;
        long lcm = lcm(a, b);
        StringBuilder result = new StringBuilder();
        while (q-- > 0) {
            long l = fs.nextLong(), r = fs.nextLong();
            long ans = util(lcm, r, b);
            if (l > 1) ans -= util(lcm, l - 1, b);
            result.append(ans).append(" ");
        }
        fw.out.println(result);
    }

    private static long util(long lcm, long r, long b) {
        if (r <= (b - 1)) return 0;
        long div = r / lcm;
        if (div == 0) return (r - (b - 1));
        long substract = b * (div - 1);
        long left = div * lcm;
        long right = Math.min(left + b - 1, r);
        substract += (right - left + 1);

        return r - (b - 1) - substract;
    }
