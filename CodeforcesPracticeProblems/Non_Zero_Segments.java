...
public class Non_Zero_Segments {
    ...
    private static void solve() {
        ...
        for (int i = 0; i < n; i++) {
            prefix_sum += arr[i];
            if (!map.containsKey(prefix_sum)) map.put(prefix_sum, new ArrayList<>());
            map.get(prefix_sum).add(i + 1);
        }
        ...
        for (List<Integer> x : map.values()) {
            if (x.size() > 1) {
                for (int i = 0; i < x.size() - 1; i++)
                    list.add(new int[]{x.get(i) + 1, x.get(i + 1)});
            }
        }
