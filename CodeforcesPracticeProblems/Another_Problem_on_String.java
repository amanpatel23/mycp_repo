...
public class Another_Problem_on_String {
    ...
    private static void solve() {
        ...
        int len = s.length();
        int[][] temp = new int[len + 5][2];
        for (int[] r : temp) Arrays.fill(r, -1);
        int ones = 0;
        long ans = 0;
        for (int i = 0; i < len; i++) {
            if (s.charAt(i) == '1') ones++;
            temp[ones][1] = i;
            if (temp[ones][0] == -1) temp[ones][0] = i;

            int diff = ones - k;
            if (diff < 0) continue;
            ans += ((k > 0 ? 1 : 0) + temp[diff][1] - temp[diff][0]);
            if (diff == 0 && temp[0][0] != -1) ans++;
        }
        fw.out.println(ans);
    }
