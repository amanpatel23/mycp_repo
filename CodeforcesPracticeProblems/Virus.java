...
public class aaaAAACodeTON_Round {
    ...
    private static void solve() {
        ...
        long[] arr = new long[m];
        for (int i = 0; i < m; i++) arr[i] = fs.nextLong() - 1;
        Arrays.sort(arr);
        List<long[]> list = new ArrayList<>();
        for (int i = 0; i < m - 1; i++) {
            list.add(new long[]{arr[i], arr[i + 1], arr[i + 1] - arr[i] - 1});
        }
        list.add(new long[]{arr[m - 1], arr[0], ((arr[0] - arr[m - 1] - 1 + n) % n)});

        list.sort(Comparator.comparingLong(x -> -x[2]));
        int day = 0;
        long ans = m;
        for (long[] temp : list) {
            long covered = (long) 2 * day;
            if (covered >= temp[2]) {
                ans += temp[2];
                continue;
            }
