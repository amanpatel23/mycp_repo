...
public class Restoring_the_Permutation {
    ...
    private static void solve() {
        ...
        int n = fs.nextInt();
        int[] prefix_max = readIntArray(n);

        int[] minimal = new int[n];
        minimal[0] = prefix_max[0];
        for (int i = 1; i < n; i++) {
            if (prefix_max[i] == prefix_max[i - 1]) continue;
            minimal[i] = prefix_max[i];
        }
        int[] maximal = minimal.clone();
        TreeSet<Integer> set = util(minimal, n);
        for (int i = 0; i < n; i++) {
            if (minimal[i] != 0) continue;
            minimal[i] = set.pollFirst();
        }

        set = util(maximal, n);
        for (int i = 0; i < n; i++) {
            if (maximal[i] != 0) continue;
            maximal[i] = set.lower(maximal[i - 1]);
            set.remove(maximal[i]);
        }

        for (int x : minimal) fw.out.print(x + " ");
        fw.out.println();
        for (int x : maximal) fw.out.print(x + " ");
        fw.out.println();
    }

    private static TreeSet<Integer> util(int[] arr, int n) {
        int[] vis = new int[n + 1];
        for (int x : arr) vis[x] = 1;
        TreeSet<Integer> set = new TreeSet<>();
        for (int i = 1; i <= n; i++) {
            if (vis[i] == 0) set.add(i);
        }
        return set;
    }
