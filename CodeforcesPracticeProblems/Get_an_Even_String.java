...
public class Get_an_Even_String {
    ...
    private static void solve() {
        ...
        for (int i = 0; i < 26; i++) list.add(new TreeSet<>());
        for (int i = 0; i < n; i++) {
            int ascii = str.charAt(i) - 'a';
            list.get(ascii).add(i);
        }

        dp = new int[n];
        Arrays.fill(dp, -1);
        int ans = dfs(0, n, str, list);
        fw.out.println(ans);
    }

    private static int dfs(int i, int n, String str, List<TreeSet<Integer>> list) {
        if (i >= n) return 0;
        if (dp[i] != -1) return dp[i];
        int ascii = str.charAt(i) - 'a';
        int ans = 1 + dfs(i + 1, n, str, list);
        Integer idx = list.get(ascii).higher(i);
        if (idx != null) {
            ans = Math.min(ans, idx - i - 1 + dfs(idx + 1, n, str, list));
        }
        return dp[i] = ans;
    }
