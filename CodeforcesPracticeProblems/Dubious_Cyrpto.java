...
public class Dubious_Crypto {
    ...
    private static void solve() {
        ...
        long l = fs.nextLong(), r = fs.nextLong(), m = fs.nextLong();
        for (long a = l; ; a++) {
            long n = (m + r - l) / a;
            if (n == 0) continue;
            long b_minus_c = m - (a * n);
            if (b_minus_c >= (l - r) && b_minus_c <= (r - l)) {
                if (b_minus_c >= 0) {
                    fw.out.println(a + " " + r + " " + (r - b_minus_c));
                }else {
                    fw.out.println(a + " " + (r + b_minus_c) + " " + r);
                }
                break;
            }
        }
