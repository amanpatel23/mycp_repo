...
public class PhoenixAndDistribution {
    ...
    private static void solve() {
        ...
        if (list_char.get(0) != list_char.get(k - 1)) {
            fw.out.println(list_char.get(k - 1));
            return;
        }

        StringBuilder result = new StringBuilder();
        result.append(list_char.get(0));
        if (map.size() > 2 || (map.size() == 2 && map.firstEntry().getValue() > k)) {
            for (int i = k; i < n; i++)
                result.append(list_char.get(i));
        } else {
            for (int i = k; i < n; i += k)
                result.append(list_char.get(i));
        }
