...
public class Air_Conditioners2 {
    ...
    private static void solve() {
        ...
        int[] ac_pos = readIntArray(k);
        int[] ac_t = readIntArray(k);

        long[] ans = new long[n + 5];
        Arrays.fill(ans, lMax);
        Queue<int[]> pq = new PriorityQueue<>(Comparator.comparingInt(o -> o[0]));
        for (int i = 0; i < k; i++) {
            int pos = ac_pos[i], t = ac_t[i];
            ans[pos] = t;
            pq.add(new int[]{t, pos});
        }
        while (!pq.isEmpty()) {
            int[] top = pq.poll();
            int t = top[0], pos = top[1];
            if (ans[pos] < t) continue;
            if (pos - 1 >= 1 && ans[pos - 1] > t + 1) {
                ans[pos - 1] = t + 1;
                pq.add(new int[]{t + 1, pos - 1});
            }
            if (pos + 1 <= n && ans[pos + 1] > t + 1) {
                ans[pos + 1] = t + 1;
                pq.add(new int[]{t + 1, pos + 1});
            }
        }
