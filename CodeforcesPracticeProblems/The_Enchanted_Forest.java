...
public class CF_796_Div2 {
    ...
    private static void solve() {
        ...
        for (int i = 1; i <= n; i++) {
            prefix_sum[i] = fs.nextInt() + prefix_sum[i - 1];
        }
        int min = Math.min(n, k);
        long sum = lMin;
        for (int i = min; i <= n; i++) {
            sum = Math.max(sum, prefix_sum[i] - prefix_sum[i - min]);
        }

        long ans = sum;
        //fw.out.println(ans);
        ans += (((long) min * (min - 1)) / 2);
        //fw.out.println(ans);
        int left = Math.max(0, k - min);
        ans += ((long) left * n);
        fw.out.println(ans);
    }

    private static int util(int x) {
        int cnt = 0;
        while ((x & 1) == 0) {
            x /= 2;
            cnt++;
