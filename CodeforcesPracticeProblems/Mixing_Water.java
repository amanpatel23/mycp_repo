...
public class Mixing_Water {
    ...
    private static void solve() {
        ...
        long h = fs.nextLong(), c = fs.nextLong(), t = fs.nextLong();
        if (t == h) {
            fw.out.println(1);
            return;
        }
        long even_times_avg = (h + c) / 2;
        if (t <= even_times_avg) {
            fw.out.println(2);
            return;
        }

        long k = Math.abs(((t - c) / ((2 * t) - h - c)));
        long temp = k * (h + c - (2 * t));
        long lhs = Math.abs(temp + t - c) * ((2 * k) + 1);
        long rhs = Math.abs(temp - t + h) * ((2 * k) - 1);
    ...
    private static double util(long x, long h, long c) {
        long sum = (h * x) + (c * (x - 1));
        return ((0.0 + sum) / (0.0 + x + x - 1));
    }
