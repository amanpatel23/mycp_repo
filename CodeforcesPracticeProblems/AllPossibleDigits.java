...
public class AllPossibleDigits {
    ...
    private static void solve() {
        ...
        int ll = arr[n - 1];
        while (set.contains(ll - 1)) ll--;
        int rr = p - 1;
        while (rr > arr[n - 1] && set.contains(rr)) rr--;
        if (ll == 0) {
            fw.out.println(rr - arr[n - 1]);
        } else {
            int ans = p - arr[n - 1];
            int carry = 1;
            for (int i = n - 2; i >= 0 && carry == 1; i--) {
                if (arr[i] == p - 1) arr[i] = 0;
                else {
                    arr[i] += 1;
                    carry = 0;
                }
