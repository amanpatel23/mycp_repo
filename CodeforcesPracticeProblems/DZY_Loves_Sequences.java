...
public class DZY_Loves_Sequences {
    ...
    private static void solve() {
        ...
        int len = 0;
        for (int i = 1; i <= n; i++) {
            dp[i][0] = 1 + (int) getMax(arr[i] > arr[i - 1] ? dp[i - 1][0] : 0,
                    (arr[i] > changed_ht)? dp[i - 1][1] : 0);
            dp[i][1] = 1 + len;
            changed_ht = arr[i - 1] + 1;
            if (arr[i] > arr[i - 1]) len++;
            else len = 1;
            ans = (int) getMax(ans, dp[i][0], dp[i][1]);
        }

        for (int i = 1; i <= n / 2; i++) {
            swapInIntArr(arr, i, n - i + 1);
        }
        //fw.out.println(Arrays.toString(arr));
        arr[0] = iMax;
        len = 0;
        changed_ht = 0;
        for (int i = 1; i <= n; i++) {
