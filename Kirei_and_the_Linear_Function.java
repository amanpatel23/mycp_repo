...
public class Kirei_and_the_Linear_Function {
    ...
    private static void solve() {
        ...
        int n = str.length();
        int w = fs.nextInt(), m = fs.nextInt();
        int[] prefix_rem = new int[n + 1];
        for (int i = 0; i < n; i++) {
            int d = str.charAt(i) - '0';
            prefix_rem[i + 1] = (prefix_rem[i] + d) % 9;
        }
        List<List<Integer>> list = new ArrayList<>();
        for (int i = 0; i <= 10; i++) list.add(new ArrayList<>());
        for (int i = w; i <= n; i++) {
            int num = (prefix_rem[i] - prefix_rem[i - w] + 9) % 9;
            list.get(num).add(i - w + 1);
        }

        while (m-- > 0) {
            int l = fs.nextInt(), r = fs.nextInt(), k = fs.nextInt();
            int xx = (prefix_rem[r] - prefix_rem[l - 1] + 9) % 9;
            int[] ans = new int[]{-1, -1};
            for (int i = 0; i <= 8; i++) {
                for (int j = 0; j <= 8; j++) {
                    int lhs = (((i * xx) % 9) + j) % 9;
                    ...
                    if (i == j) {
                        ...
                        int ff = list.get(i).get(0), ss = list.get(i).get(1);
                        util(ans, ff, ss);
                    } else {
                        if (list.get(i).size() == 0 || list.get(j).size() == 0) continue;
                        int ff = list.get(i).get(0), ss = list.get(j).get(0);
                        util(ans, ff, ss);
                    }
                }
            }

            fw.out.println(ans[0] + " " + ans[1]);
        }
    }

    private static void util(int[] ans, int ff, int ss) {
        if (ans[0] == -1 || ans[0] > ff) {
            ans[0] = ff;
            ans[1] = ss;
            return;
        }
        if (ans[0] == ff && ans[1] > ss) {
            ans[1] = ss;
        }
    }
