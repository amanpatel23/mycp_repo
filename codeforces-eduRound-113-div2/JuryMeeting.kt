...
private class JuryMeeting {
    ...
    fun solveTestCase() {
        ...
        for (i in 0 until n)
            problems.add(readInt())

        problems.sort()
        val totalP = fact[n]

        if (problems[n - 1] == problems[n - 2])
            out.println(totalP)
        else {
            if (problems[n - 1] - problems[n - 2] == 1) {
                var count = problems.count { it == problems[n - 2] }
                var result = totalP
                for (i in 0 until n) {
                    if (i < count)
                        continue

                    val decrease = ((nCr(i, count) * fact[count]) % mod * fact[n - count - 1]) % mod
                    result = (result - decrease) % mod
                    if (result < 0)
                        result += mod
                }
                out.println(result)
