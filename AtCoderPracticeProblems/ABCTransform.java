...
public class ABCTransform {
    ...
    private static final String[] arr = new String[]{"BC", "CA", "AB"};
    private static void solve() {

        s = fs.nextLine();
        int q = fs.nextInt();
        while (q-- > 0) {
            long t = fs.nextLong(), k = fs.nextLong() - 1;
            char result = getChar(t, k);
            fw.out.println(result);
        }
    }

    private static char getChar(long t, long k) {

        if (t == 0)
            return s.charAt((int) (k));
        if (k == 0) {
            return (char) ('A' + (char) (((s.charAt(0) - 'A') + t) % 3));
        }

        char temp = getChar(t - 1, k / 2);
        return arr[temp - 'A'].charAt((int) (k & 1));
    }
