...
public class Main {
    ...
    private static void solve() {
        ...
        for (int i = 0; i < n; i++) points[i] = new int[]{fs.nextInt(), fs.nextInt()};
        if (k == 1) {
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                ...
                for (int ii = j + 1; ii < n; ii++) {
                    if (collinear(i, j, ii, points)) list.add(ii);
                }

                if (list.size() >= k) ans++;
                for (int ii = 0; ii < list.size(); ii++) {
                    for (int jj = ii + 1; jj < list.size(); jj++) {
                        flag[list.get(ii)][list.get(jj)] = false;
                    }
                }
            }
        }
    ...
    private static boolean collinear(int a, int b, int c, int[][] points) {
        long val1 = ((long) points[b][1] - points[a][1]) * ((long) points[c][0] - points[a][0]);
        long val2 = ((long) points[c][1] - points[a][1]) * ((long) points[b][0] - points[a][0]);
        return (val1 == val2);
    }
