...
public class Main {
    ...
    private static void solve() {
        ...
        for (int i = 1; i <= n; i++) list.get(fs.nextInt()).add(i);

        int q = fs.nextInt();
        while (q-- > 0) {
            int l = fs.nextInt(), r = fs.nextInt(), x = fs.nextInt();
            int idx1 = lower_bound(list.get(x), l);
            int idx2 = upper_bound(list.get(x), r);
            if (idx1 == -1 || idx2 == -1) fw.out.println(0);
            else fw.out.println(idx2 - idx1 + 1);
        }
    }
    ...
    private static int lower_bound(List<Integer> list, int x) {
        ...
        else if (list.get(r) >= x) return r;
        else return -1;
    }

    private static int upper_bound(List<Integer> list, int x) {
        if (list.size() == 0) return -1;
        int l = 0, r = list.size() - 1;
        while (r - l > 1) {
            int mid = l + ((r - l) / 2);
