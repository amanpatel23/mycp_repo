...
public class Main {
    ...
    private static final long[] power_of_26 = new long[(int) (1e6)];
    public static void main(String[] args) throws IOException {
        ...
        power_of_26[0] = 1;
        for (int i = 1; i <= (int) ((1e6) / 2) + 5; i++) {
            power_of_26[i] = (power_of_26[i - 1] * 26) % mod2;
        }
    ...
    private static void solve() {
        ...
        if ((n & 1) == 0)
            div--;
        StringBuilder last_one = new StringBuilder();
        long result = 0;
        for (int i = 0; i <= div; i++) {
            last_one.append(s.charAt(i));
            int num = s.charAt(i) - 'A';
            //fw.out.println(num);
            long curr = (num * power_of_26[div - i]) % mod2;
            result = (result + curr) % mod2;
        }

        int i = ((n & 1) == 1) ? (div - 1) : div;
        for (; i >= 0; i--)
            last_one.append(s.charAt(i));

        result += (last_one.toString().compareTo(s) <= 0) ? 1 : 0;
        result %= mod2;
