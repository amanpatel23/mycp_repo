...
public class Main {
    ...
    private static void solve() {
        ...
        vertex_val = new int[n + 1];
        for (int i = 1; i <= n; i++)
            vertex_val[i] = fs.nextInt();
        ...
        for (int i = 0; i < (n - 1); i++)
            addEdge(fs.nextInt(), fs.nextInt());

        dfs(1, 1);
        while (q-- > 0) {
            int v = fs.nextInt(), k = fs.nextInt() - 1;
            int result = result_list.get(v).get(k);
    ...
    private static List<Integer> dfs(int curr, int parent) {

        List<Integer> temp = new ArrayList<>();
        temp.add(vertex_val[curr]);

        for (int x : adjList.get(curr)) {
            if (x != parent) {
                temp.addAll(dfs(x, curr));
            }
        }

        temp.sort(Comparator.reverseOrder());
        int size = temp.size();
        for (int i = 1; i <= Math.min(20, size); i++)
            result_list.get(curr).add(temp.get(i - 1));

        return result_list.get(curr);
    }
